 ...
 **/
function UploadAndRenderEle(index, componentName, res, allData,text,text_bg,qipao_scale,qipao_text) {
    //获取当前舞台容器
    ...
    PIXI.loader.load(function (loader, resource) {
        ...
        var initData = {
            ...
            "zText":text,
            "textBg":text_bg,
            "qipao_scale":qipao_scale,
            "qipao_text":qipao_text
            //根据判断是否需要修改当前元素
        };if (element) {
            //移除舞台对应元素
            ...
            if (key != 'id' && key != 'bgImage' && key != 'subTitle' && key != 'stemImg' && key != 'rightKey') {
                ...
                resourceNameData.forEach(function (item, ids) {
                    var initData = {
                        ...
                        "zText":item.text,
                        "textBg":item.text_bg,
                        "qipao_scale":item.qipao_scale,
                        "qipao_text":item.qipao_text
                    };
            ...
            if (key != 'id' && key != 'bgImage' && key != 'subTitle' && key != 'stemImg' && key != 'rightKey') {
                ...
                for (var i = resourceNameData.length - 1; i >= 0; i--) {
                    var initData = {
                        ...
                        "zText":resourceNameData[i].text,
                        "textBg":resourceNameData[i].text_bg,
                        "qipao_scale":resourceNameData[i].qipao_scale,
                        "qipao_text":resourceNameData[i].qipao_text
                    };
                    renderEleByType(initData, allData, true);
                }
            }
...
function renderEleByType(obj, allData, editStatus) {
    ...
    if (obj.resourceName) {
        if (obj.resourceName.indexOf('image') > -1 && (obj.show || obj.resourceName.indexOf('render') > -1)) {
            ...
            // 渲染动画元素
            if(obj.zText !== undefined && obj.textBg !== undefined && obj.qipao_scale !== undefined && obj.qipao_text !== undefined){
                // 渲染带有文字的动画元素
                console.log("渲染动画元素==============",obj);
                let sprite = new PIXI.Sprite.fromImage(res[obj.textBg].url);
                sprite.anchor.set(.5,.5);
                sprite.scale.set(obj.qipao_scale,obj.qipao_scale);
                let textSprit = null;
                if(obj.qipao_scale == 1){
                    textSprit = new PIXI.Text(obj.zText,{fontSize:48,fontFamily:'Microsoft YaHei',fill:'#FFFFFF',fontWeight:'bold',align:'center'});
                }else if(obj.qipao_scale == 1.5){
                    textSprit = new PIXI.Text(obj.zText,{fontSize:obj.qipao_text,fontFamily:'Microsoft YaHei',fill:'#FFFFFF',fontWeight:'bold',align:'center'});
                }
                textSprit.anchor.set(.5,.5);
                textSprit.y = -10;
                sprite.name = 'paopao';
                sprite.addChild(textSprit);
